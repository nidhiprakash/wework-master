...
session_start();
  ...
  include("config.php");
/*$sql2 = "SELECT * FROM `jobapplicant` WHERE applicantid = '$_SESSION[user_id]' ";
$result=mysqli_query($db,$sql2);
$i=0;
while($row = $result->fetch_assoc()){
  $row_array[$i]['actualjobid']=$row['actualjobid'];
}
$actualjobid=mysqli_query($db,$sql2)->fetch_object()->actualjobid;
$j=0;
$return_arr = array();
  while ($row = $result->fetch_assoc()) 
  {
  ...
  $i=0;
   $sql = "SELECT * FROM job LEFT JOIN jobapplicant ON job.jobid=jobapplicant.actualjobid WHERE jobapplicant.applicantid='$userid' ";
   $result = mysqli_query($db,$sql);
      ...
      $sql2 = "SELECT * FROM job LEFT JOIN jobapplicant ON job.jobid=jobapplicant.actualjobid WHERE jobapplicant.applicantid='$userid'";
      $result=mysqli_query($db,$sql2);
      $i=0;
      $return_arr = array();
      while ($row = $result->fetch_assoc()) 
  ...
  <div class="container bg-grey" style="margin-top: 50px; height: 250px;">
              <div class="row">
                ...
                <div class="col-3">
                  ...
                  <br>
                  <button class="btn btn-common" style="margin-left: 850px; margin-top: 20px; margin-bottom: 30px; width: 200px;" name="status" value="<?php echo $row_array[$j]['status'] ?>"><?php echo $row_array[$j]['status'] ?></button>
                </div>
              </div>
