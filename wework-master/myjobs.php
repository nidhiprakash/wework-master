...
$jobid="";
  include("config.php");
$sql2 = "SELECT * FROM `job` WHERE postername = '$_SESSION[user_id]' ";
...
$return_arr = array();
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $status="complete";
    $sql = "UPDATE job SET status='$status' WHERE jobid = '$jobid' ";
    $result1=mysqli_query($db,$sql);
    $sql2 = "UPDATE jobapplicant SET status='$status' WHERE actualjobid='$jobid'";
    $result=mysqli_query($db,$sql2);
  }
                ...
                <div class="col-3">
                  ...
                  <form role="form" action="delete_job.php" method="POST">
                    <input type="hidden" value="<?php echo $row_array[$j]['jobid']; ?>">
          		    <button class="btn btn-common" onclick="myFunction1()" style="margin-left: 850px; margin-top: 20px; margin-bottom: 30px; width: 200px;" name="jobid" value="<?php echo $row_array[$j]['jobid'] ?>">Delete</button>
                </form>
                <br>
                <form role="form" action="select_applicant.php" method="POST">
                    <input type="hidden"  value="<?php echo $row_array[$j]['jobid']; ?>">
                  <button class="btn btn-common" onclick="myFunction1()" style="margin-left: 600px;  margin-top: -235px; width: 200px; color: #fec239;" name="jobid" value="<?php echo $row_array[$j]['jobid'] ?>">Select Applicant</button>
                </form>
                <br>
                <form role="form" action="payment.php" method="POST">
                    <input type="hidden" name="jobid" value="<?php echo $row_array[$j]['jobid']; ?>">
                  <button class="btn btn-common" onclick="myFunction1()" style="margin-left: 600px;  margin-top: -205px; width: 200px; color: #fec239;" name="jobid" value="<?php echo $row_array[$j]['jobid'] ?>">Pay</button>
                </form>
                </div>
              </div>
